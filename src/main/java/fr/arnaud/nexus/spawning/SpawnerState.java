 ...
 * wave is currently active, timers, and kill counts. It holds no logic — all
 * decisions are made by {@link MobSpawnManager}.
 */
public final class SpawnerState {
    ...
    /**
     * Elapsed seconds since the current KILL wave started. Used to enforce timeout.
     * Reset to 0 when a new wave begins.
     */
    private float killWaveTimeoutTimer = 0f;

    /**
     * Total mobs spawned in the current wave, keyed by mob entry index.
     */
    private final Map<Integer, Integer> spawnedCountPerEntry = new HashMap<>();

    /**
     * Mobs still alive from the current wave. Tracked by the spawn manager.
     */
    private int aliveMobsInCurrentWave = 0;

    /**
     * Total mobs that were spawned in the current wave (used to compute kill percentile).
     */
    private int totalMobsInCurrentWave = 0;

    /**
     * Accumulated time for per-entry spawnRate drip. Keyed by mob entry index.
     */
    private final Map<Integer, Float> spawnRateAccumulatorPerEntry = new HashMap<>();

    /**
     * Accumulated pending spawns waiting on spawnRate drip. Keyed by mob entry index.
     */
