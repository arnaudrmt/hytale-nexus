...
public final class WaveBarSystem extends EntityTickingSystem<EntityStore> {
    ...
    public void tick(float dt, int index, ArchetypeChunk<EntityStore> chunk,
        ...
        if (entry.completeTimer()[0] > 0f) {
            ...
            if (entry.completeTimer()[0] <= 0f) {
                ...
                entry.world().execute(() -> Nexus.get().getNexusHudSystem().hideWave(ref));
            }
            return;
        }
        ...
        WaveBarStateProvider.WaveSnapshot snap =
            Nexus.get().getWaveBarStateProvider().getSnapshot(nearest.getId());
        if (snap == null) return;
        ...
        if (snap.completed() && !entry.lastCompleted()[0]) {
            ...
            entry.world().execute(() -> Nexus.get().getNexusHudSystem().showWaveComplete(ref));
            return;
        }
        ...
        entry.world().execute(() -> Nexus.get().getNexusHudSystem().showWave(ref, cw, tw, k, t));
    }
    ...
    private static SpawnerState findNearestActiveSpawner(Vector3d pos, int completingSpawnerId) {
        ...
        for (SpawnerState state : Nexus.get().getMobSpawnerManager().getSpawnerStates()) {
            if (!state.isTriggered()) continue;
            if (state.isComplete() && state.getId() != completingSpawnerId) continue;

            LevelConfig.Position p = state.getConfig().getPosition();
