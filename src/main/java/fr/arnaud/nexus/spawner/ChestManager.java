...
public final class ChestManager {
    ...
    public void onLevelLoaded(World world, LevelConfig config) {
        ...
        int id = 0;
        for (LevelConfig.IndependentChestConfig chestConfig : config.independentChests()) {
    ...
    private void checkProximityTrigger(IndependentChestState state, Vector3d playerPosition) {
        ...
        float r = state.getConfig().triggerRadius();
    ...
    private void spawnChest(IndependentChestState state) {
        ...
        LootChestConfig wrapper = new LootChestConfig(config.items());
    ...
    private void unlockCore(String itemId, Ref<EntityStore> playerRef, Store<EntityStore> store) {
        ...
        if (playerRefComponent != null) {
            EventTitleUtil.showEventTitleToPlayer(
                playerRefComponent,
                Message.translation("nexus.core.unlocked.title").param("ability", Message.translation(ability.getDisplayNameKey())),
    ...
    private ItemStack buildLootStack(String itemId) {
        if (isNexusWeapon(itemId)) {
            ...
            if (item != null) {
                ...
                if (doc != null) {
                    return new ItemStack(itemId, 1, doc);
                }
            }
        }
    ...
    void spawnAuraSphere(Vector3d pos) {
        ...
        activeWorld.getPlayerRefs().forEach(_ -> {
